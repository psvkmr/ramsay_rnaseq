---
title: "S303 - Control v CLL LN excluding sample NAT10"
...
# Exlcuding NAT 10 in this run as potential outlier?
md <- md[!md$sample_id %in% c('NAT 4', 'NAT 10'), ]

# Also drop from CM
zcm <- zcm[, !colnames(zcm) %in% 'NAT.10_S10']
ncm <- ncm[, !colnames(ncm) %in% 'NAT.10_S10']
...
```{r nsamples}
length(mds$ctrl_v_cll_ln$sample_id)
```

IDs of the samples used in the analysis

```{r smpls}
mds$ctrl_v_cll_ln$sample_id
```

Number of samples split by contrast group

```{r grps}
table(mds$ctrl_v_cll_ln$sample_group)
```
...
optimumScree <- function(pca, svcm, ttl){
  ...
  plt <- screeplot(pca,
            components = getComponents(pca, 1:7),
...
eigencorr2.plt$ctrl_v_cll_ln
```


```{r dds, message=FALSE}
#deseq.dataset <- DESeqDataSetFromMatrix(countData = nat.cm, 
deseq.dataset <- DESeqDataSetFromMatrix(countData = zcms$ctrl_v_cll_ln[apply(zcms$ctrl_v_cll_ln, 1, function(x) all(x !=0 )), ],
                       colData = mds$ctrl_v_cll_ln, 
                       design = ~ sample_group)
```
...
#write_csv(deseq.dfs$cll_ln, 'C:/Users/Prasanth/google_drive/guys/ramsey/s303_control_v_cllln_excluding_NAT10S10.csv')

head(deseq.dfs$cll_ln, n = 5)
```
...
```{r up}
strongly.up <- filter(deseq.dfs$cll_ln, padj < 0.05 & log2FoldChange > log2(1.5))$external_gene_name
...
```{r sup}
slightly.up <- filter(deseq.dfs$cll_ln, padj < 0.05 & log2FoldChange > 0 & log2FoldChange <= log2(1.5))$external_gene_name
...
```{r sdown}
slightly.down <- filter(deseq.dfs$cll_ln, padj < 0.05 & log2FoldChange < 0 & fold_change >= -log2(1.5))$external_gene_name
...
```{r down}
strongly.down <- filter(deseq.dfs$cll_ln, padj < 0.05 & log2FoldChange < -log2(1.5))$external_gene_name
...
runGo <- function(gl, srce, ordr = TRUE){
  gost(query = gl,
                ...
                domain_scope = "annotated", custom_bg = deseq.dfs$cll_ln$external_gene_name, 
...
coalldf <- coalldf[!coalldf$term_id == 'CORUM:1119', ]
