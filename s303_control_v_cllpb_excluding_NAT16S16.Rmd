---
title: "S303 - Control v CLL PB excluding sample NAT 16"
...
# Exlcuding NAT 16 in this run as potential outlier?
md <- md[!md$sample_id %in% c('NAT 4', 'NAT 16'), ]

# Also drop from CM
zcm <- zcm[, !colnames(zcm) %in% 'NAT.16_S16']
ncm <- ncm[, !colnames(ncm) %in% 'NAT.16_S16']
...
```{r nsamples}
length(mds$ctrl_v_cll_pb$sample_id)
```

IDs of the samples used in the analysis

```{r smpls}
mds$ctrl_v_cll_pb$sample_id
```

Number of samples split by contrast group

```{r grps}
table(mds$ctrl_v_cll_pb$sample_group)
```
...
eigencorr2.plt$ctrl_v_cll_pb
```


```{r dds, message=FALSE}
#deseq.dataset <- DESeqDataSetFromMatrix(countData = nat.cm, 
deseq.dataset <- DESeqDataSetFromMatrix(countData = zcms$ctrl_v_cll_pb[apply(zcms$ctrl_v_cll_pb, 1, function(x) all(x !=0 )), ],
                       colData = mds$ctrl_v_cll_pb, 
                       design = ~ sample_group)
```
...
#write_csv(deseq.dfs$cll_pb, 'C:/Users/Prasanth/google_drive/guys/ramsey/s303_control_v_cllpb_excluding_NAT16S16.csv')

head(deseq.dfs$cll_pb, n = 5)
```
...
```{r up}
strongly.up <- filter(deseq.dfs$cll_pb, padj < 0.05 & log2FoldChange > log2(1.5))$external_gene_name
...
```{r sup}
slightly.up <- filter(deseq.dfs$cll_pb, padj < 0.05 & log2FoldChange > 0 & log2FoldChange <= log2(1.5))$external_gene_name
...
```{r sdown}
slightly.down <- filter(deseq.dfs$cll_pb, padj < 0.05 & log2FoldChange < 0 & fold_change >= -log2(1.5))$external_gene_name
...
```{r down}
strongly.down <- filter(deseq.dfs$cll_pb, padj < 0.05 & log2FoldChange < -log2(1.5))$external_gene_name
...
runGo <- function(gl, srce, ordr = TRUE){
  gost(query = gl,
                ...
                domain_scope = "annotated", custom_bg = deseq.dfs$cll_pb$external_gene_name, 
