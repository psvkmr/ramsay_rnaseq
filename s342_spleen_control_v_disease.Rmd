---
title: "S342 TCL1 Spleen Disease v Ctrl"
...
```{r load_cms}
# load count matrices
load('ramsey_s342_norm_counts_matrix.RData')
load('ramsey_s342_non_zero_counts_matrix.RData')
```

```{r load_md}
# load metadata
md <- read.csv('s342_metadata.csv')
```

```{r ids}
ids <- read.csv('C:/Users/Prasanth/google_drive/guys/resources/mouse_biomart_unique_ensembl_genes.csv')
```

```{r modify_md}
# add matrix sample names to metadata variable as different
md$matrix_id <- gsub('-', '.', md$sample)
...
md$sampleName <- gsub('_.*$', '', md$sampleName)
md$sampleName <- gsub('\\.', '_', md$sampleName)
```
...
smpls <- rownames(md[md$MouseModel == 'TCL1' & md$Tissue == 'SP', ])
nat.cm <- zcm[, smpls]
nat.ncm <- ncm[, smpls]
nat.md <- md[smpls, ]

# sample groups
smpl.grps <- list(Control = rownames(nat.md[nat.md$Disease == 'N', ]), Disease = rownames(nat.md[nat.md$Disease == 'Y', ]))
mds <- nat.md
```

```{r md}
# subset metadata by comparisons
mds$Disease <- factor(mds$Disease, levels = c('N', 'Y'))
```

The number of samples used in this analysis

```{r nsamples}
length(mds$sampleName)
```
...
zcms <- nat.cm[rowSums(nat.cm) > (((dim(nat.cm)[2])/2)-1), ]
dim(zcms)
```
...
svcms <- varianceStabilizingTransformation(zcms)
...
pcas <- pca(svcms, metadata = mds, removeVar = 0.1)
```
...
optimumScree(pcas, svcms, 'Control v Disease')
```
...
bi.plt.smpls <- 
  biplot(pcas, showLoadings = F, colby = 'Disease', 
                                  ...
                                  axisLabSize = 8, borderWidth = 0.5, xlim = c(-150, 150), ylim = c(-100, 100))
bi.plt.smpls
```
...
```{r dds, message=FALSE}
#deseq.dataset <- DESeqDataSetFromMatrix(countData = zcms[apply(zcms, 1, function(x) all(x !=0 )), ],
#                       colData = mds, 
#                       design = ~ Disease)
deseq.dataset <- DESeqDataSetFromMatrix(countData = zcms,
                       colData = mds, 
                       design = ~ Disease)
...
select <- order(rowMeans(counts(deseq.analysis, normalized=T)),
                  decreasing=T)[1:10]
df <- as.data.frame(colData(deseq.analysis)[, 'Disease'], row.names = row.names(colData(deseq.analysis))) %>% `names<-`('Group')
...
rownames(sampleDistMatrix) <- paste(vsd$Disease, vsd$sampleName, sep="-")
colnames(sampleDistMatrix) <- NULL
pheatmap(sampleDistMatrix,
         clustering_distance_rows=sampleDists,
         clustering_distance_cols=sampleDists, fontsize_row = 8)
```
...
#write_csv(deseq.dfs, 'C:/Users/Prasanth/google_drive/guys/ramsey/s342_spleen_control_v_disease.csv')
...
```{r up}
strongly.up <- filter(deseq.dfs, padj < 0.05 & log2FoldChange > log2(1.5))$external_gene_name
...
```{r sup}
slightly.up <- filter(deseq.dfs, padj < 0.05 & log2FoldChange > 0 & log2FoldChange <= log2(1.5))$external_gene_name
...
```{r sdown}
slightly.down <- filter(deseq.dfs, padj < 0.05 & log2FoldChange < 0 & log2FoldChange >= -log2(1.5))$external_gene_name
...
```{r down}
strongly.down <- filter(deseq.dfs, padj < 0.05 & log2FoldChange < -log2(1.5))$external_gene_name
...
The differentially expressed genes can be contextualised by examining their impacts in different tissues, pathways, interactions and biological processes. Below are enrichments of the differentially expressed genes in various gene sets. Enrichments are analysed for  significantly upregulated genes,  significantly downregulated genes, or for all signficantly dysregulated genes together regardless of fold change. If a plot is missing for any of these groups in these contextualisations, there were no significant overlaps in this case.
...
runGo <- function(gl, srce, ordr = TRUE){
  gost(query = gl,
                organism = "mmusculus", ordered_query = ordr, 
                significant = FALSE, exclude_iea = FALSE, 
                measure_underrepresentation = FALSE, evcodes = FALSE, 
                user_threshold = 0.05, correction_method = "g_SCS", 
                domain_scope = "annotated", custom_bg = deseq.dfs$external_gene_name, 
...
```{r go, message=FALSE}
goup <- runGo(c(strongly.up, slightly.up), 'GO')
goupdf <- dfGo(goup$result)
plotGo(goupdf, 'GO terms - significantly upreguated genes')

godown <- runGo(c(strongly.down, slightly.down), 'GO')
godowndf <- dfGo(godown$result)
plotGo(godowndf, 'GO terms - significantly downregulated genes')
...
pathup <- runGo(c(strongly.up, slightly.up), c('KEGG', 'REAC', 'WP'))
pathupdf <- dfGo(pathup$result)
plotGo(pathupdf, 'Pathways - significantly upregulated genes')

pathdown <- runGo(c(strongly.down, slightly.down), c('KEGG', 'REAC', 'WP'))
pathdowndf <- dfGo(pathdown$result)
plotGo(pathdowndf, 'Pathways - significantly downregulated genes')
...
```{r tr, message=FALSE}
trup <- runGo(c(strongly.up, slightly.up), c('TF'))
trupdf <- dfGo(trup$result)
#plotGo(trupdf, 'Transcription factors - significantly upregulated genes')

trdown <- runGo(c(strongly.down, slightly.down), c('TF'))
trdowndf <- dfGo(trdown$result)
#plotGo(trdowndf, 'Transcription factors - significantly downregulated genes')
...
No significant enrichments here

```{r mirna, message=FALSE, echo=FALSE, include=FALSE, warning=FALSE}
# nothing interesting found so exclude
miup <- runGo(c(strongly.up, slightly.up), c('MIRNA'))
miupdf <- dfGo(miup$result)
plotGo(miupdf, 'miRNA-target interactions - significantly upregulated genes')
#miupdf[, c(1, 8)]

midown <- runGo(c(strongly.down, slightly.down), c('MIRNA'))
midowndf <- dfGo(midown$result)
plotGo(midowndf, 'miRNA-target interactions - significantly downregulated genes')
#midowndf[, c(1, 8)]
...
```{r hpa, message=FALSE, include=FALSE, echo=FALSE, warning=FALSE}
# currently not working, query fails
...
```{r corum, message=FALSE, echo=FALSE, warning=FALSE, include=FALSE}
# nothing interesting found so exclude from report
coup <- runGo(c(strongly.up, slightly.up), c('CORUM'))
coupdf <- dfGo(coup$result)
#plotGo(coupdf, 'Protein complexes - significantly upregulated genes')

codown <- runGo(c(strongly.down, slightly.down), c('CORUM'))
codowndf <- dfGo(codown$result)
#plotGo(codowndf, 'Protein complexes - significantly downregulated genes')
...
```{r hpo, message=FALSE}
hpoup <- runGo(c(strongly.up, slightly.up), c('HP'))
hpoupdf <- dfGo(hpoup$result)
#plotGo(hpoupdf, 'Phenotype onotology - significantly upregulated genes')
#hpoupdf[, c(1, 8)]

hpodown <- runGo(c(strongly.down, slightly.down), c('HP'))
hpodowndf <- dfGo(hpodown$result)
#plotGo(hpodowndf, 'Phenotype onotology - significantly downregulated genes')
#hpodowndf[, c(1, 8)]
